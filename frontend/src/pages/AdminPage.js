...
const AdminPage = () => {
  ...
  const [message, setMessage] = useState("");
  const [messageContent, setMessageContent] = useState({});
  const [newUser, setNewUser] = useState({ name: "", email: "", password: "" }); // Ny användare för registrering
  const API_URL = process.env.REACT_APP_API_URL; // Base API URL from environment variables
  ...
  const handleCreateUser = async (e) => {
    ...
    try {
      await axios.post(`${API_URL}/api/users/register`, newUser, config);
  ...
  return (
    <Container className="admin-container mt-5">
      ...
      <div className="admin-section mb-5">
        ...
        {users.length > 0 ? (
          <Table striped bordered hover responsive className="admin-table mt-5">
            <thead>
              <tr>
                <th>Name</th>
                <th>Email</th>
